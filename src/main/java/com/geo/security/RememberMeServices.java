...
import com.geo.config.SecurityConfiguration;
...
public class RememberMeServices extends AbstractRememberMeServices {
	...
	public RememberMeServices(Environment env,
		...
		super(SecurityConfiguration.REMEMBER_ME_KEY, userDetailsService);
		super.setParameter("remember-me");
		random = new SecureRandom();
		log.debug("inside RememberMeServices");
	}
	...
	public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
		...
		if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
			try {
				...
				Token token = getPersistentToken(cookieTokens);
				tokenRepo.deleteById(token.getSeries());
	...
	private Token getPersistentToken(String[] cookieTokens) {
		...
		Optional<Token> tokenOption = null;
		try {
			tokenOption = tokenRepo.findById(presentedSeries);
		...
		log.info("presentedToken={} / tokenValue={}", presentedToken, tokenOption.get().getValue());
		if (!presentedToken.equals(tokenOption.get().getValue())) {
			// Token doesn't match series value. Delete this session and throw
			// an exception.
			tokenRepo.deleteById(tokenOption.get().getSeries());
			throw new CookieTheftException(
					"Invalid remember-me token (Series/token) mismatch. Implies previous cookie theft attack.");
		}

		if (DateUtils.addDays(tokenOption.get().getDate(), TOKEN_VALIDITY_DAYS).before(new Date())) {
			tokenRepo.deleteById(tokenOption.get().getSeries());
			throw new RememberMeAuthenticationException("Remember-me login has expired");
		}
		return tokenOption.get();
	}
